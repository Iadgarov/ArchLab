...
static void assemble_program(char *program_name)
{
	...
	asm_cmd(LD,  4, 0, 2, 0); // 3: R4 = MEM[15]
	asm_cmd(ADD, 3, 3, 4, 0); // 4: R3 = R3 + R4
	asm_cmd(ADD, 2, 2, 1, 1); // 5: R2 = R2 + Immediate = R2 + 1
	asm_cmd(JLT, 0, 2, 5, 3); // 6: If (R2 < R5) then PC = 3
	asm_cmd(ST,  0, 3, 1, 15); // 7: MEM[R1] = R3
